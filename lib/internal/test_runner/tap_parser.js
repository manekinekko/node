  ...
  #parseChunk(chunk, callback = null) {
    ...
    if (astNode) {
      ...
      if (astNode.kind === TokenKind.TAP_TEST_POINT) {
        ...
        // If we managed to collect all the diagnostics data for the previous
        // test point, we can emit it.
        if (astNode.node.diagnostics?.length > 0) {
          this.#flushTestPointBuffer();
          callback?.(astNode);
        }
        else {
          // Otherwise, we need to buffer the current test point
          this.#bufferTestPoint(astNode);
        }
      } else {
        // Emit all nodes except EOF
        if (astNode.kind !== TokenKind.EOF) {
          callback?.(astNode);
        }

        else if (astNode.kind === TokenKind.EOF) {
          if (this.#bufferedTestPoints.length > 0) {
            // Flush the buffered test points
            this.#flushTestPointBuffer();
          }
        }
      }
