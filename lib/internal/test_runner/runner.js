...
class FileTest extends Test {
  #buffer = [];
  #handleReportItem({ kind, node, level = 0, lexeme, tokens }) {

    const indent = StringPrototypeRepeat(kDefaultIndent, level + 1);
    ...
    switch (kind) {
      ...
      case TokenKind.TAP_TEST_NOTOK:
        const { todo, skip, pass, fail } = node.test.status;
        const testName = node.test.description;
        const testnumber = node.test.id;
        let directive = {};

        if (todo) {
          this.skip(node.test.reason);
          directive = this.getTodo(node.test.reason);
        }

        if (skip) {
          this.todo(node.test.reason);
          directive = this.getSkip(node.test.reason);
        }

        if (pass) {
          this.reporter.ok(indent, testnumber, testName, null, directive);
        } else {
          this.reporter.fail(indent, testnumber, testName, null, directive);
        }
        break;
      ...
      case TokenKind.UNKNOWN:
        // TODO(manekinekko): figure out a better way to handle invalid TAP output
        this.reporter.invalid(indent, node.value);
        break;
    }
  }
  addToReport(ast) {
